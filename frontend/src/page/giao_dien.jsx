...
import { locationGuides } from "./locationGuides.js";
...
const SearchResults = () => {
  ...
  <div className="vnbk-filter-box">
    ...
    .filter((hotel) => {
      const city = removeDiacritics(hotel.city).toLowerCase().trim();
      const name = removeDiacritics(hotel.name).toLowerCase().trim();
      const address = removeDiacritics(hotel.address).toLowerCase().trim();
      const location = removeDiacritics(effectiveLocation).toLowerCase().trim();
      const key = removeDiacritics(keyword).toLowerCase().trim();

      const matchLocation = key !== "" || location === "" || city.includes(location);
      const matchKeyword = key === "" || name.includes(key) || city.includes(key) || address.includes(key);
      const matchPrice = hotel.price <= maxPrice;
  const matchStars = selectedStars.length === 0 || selectedStars.includes(hotel.stars);
  return matchLocation && matchKeyword && matchPrice && matchStars;
  })
    .map((hotel) => (
                    <div 
                    ...
                    >
                      ...
                      <img src={`http://localhost:5000${hotel.image}`} alt={hotel.name} className="hotel-img" />
                      </div>
      ...
      {currentGuide.topHotels.map((item, i) => (
        <div key={i} className="guide-hotel-block">
          ...
          <p className="guide-hotel-paragraph">
            <strong>{item.categoryLabel}</strong> đầu tiên mà Vietnam Booking muốn giới thiệu đến bạn chính là{" "}
            <strong className="guide-hotel-name-inline">{item.brandName}</strong>. {item.desc}
          </p>

          {item.image && (
            <figure className="guide-hotel-figure">
              <img src={item.image} alt={item.name} className="guide-hotel-img" />
            </figure>
          )}
