...
const statusLabel = (s) =>
  ({ approved: "Đã duyệt", pending: "Chờ duyệt", banned: "Bị cấm" })[s] || s;
const statusClass = (s) =>
  ({
...
export default function HotelManagement() {
  ...
  const fetchImages = async (hotelId) => {
    ...
    try {
      const res = await fetch(`${API}/api/hotels/${hotelId}/images`);
      const data = await res.json();
      setImages(data.data || []);
    } catch {
      setImages([]);
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/hotels/${selectedHotel.id}/images`, {
        method: "POST",
        body: formData,
      });
      if (res.ok) {
        showToast("success", "Tải ảnh lên thành công!");
        fetchImages(selectedHotel.id);
      } else {
  ...
  const handleDeleteImage = async (imageId) => {
    ...
    try {
      const res = await fetch(
        `${API}/api/hotels/${selectedHotel.id}/images/${imageId}`,
        {
          method: "DELETE",
        },
      );
      ...
      if (data.success) {
        ...
        showToast("error", data.message || "Xoá ảnh thất bại!");
      }
    } catch {
      showToast("error", "Lỗi kết nối server!");
    }
  ...
  const handleSubmit = async () => {
    if (!form.name.trim() || !form.city.trim() || !form.address.trim()) {
      setFormError("Vui lòng điền đầy đủ tên, thành phố và địa chỉ!");
      return;
    }
    setFormLoading(true);
    setFormError("");
  ...
  const handleAction = async (id, action) => {
    try {
      const res = await fetch(`${API}/api/admin/hotels/${id}/${action}`, {
        method: "PUT",
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${API}/api/admin/hotels/${id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.success) {
  ...
  const filtered = hotels.filter((h) => {
    ...
    const matchOwnership = isAdmin || isOwner;
  ...
  return (
    <div className="hm-root">
      {/* Toast */}
      {actionMsg && (
        <div className={`hm-toast hm-toast--${actionMsg.type}`}>
          {actionMsg.text}
        </div>
      )}
        ...
        <div className="hm-overlay" onClick={closeModal}>
          <div className="hm-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="hm-form-grid">
                <div className="hm-field hm-field--full">
                  ...
                  <input
                    ...
                    placeholder="VD: Pullman Vũng Tàu"
                  />
                </div>
                ...
                <div className="hm-field hm-field--full">
                  ...
                  <input
                    ...
                    value={form.address}
                    onChange={handleFormChange}
                    placeholder="VD: 33 Thùy Vân, Phường 8..."
                  />
                </div>
                <div className="hm-field">
                  <label>Giá/đêm (₫)</label>
            ...
            <div className="hm-modal-footer">
              <button className="hm-btn hm-btn--refresh" onClick={closeModal}>
                Huỷ
              </button>
              <button
                className="hm-btn hm-btn--approve"
      ...
      <div className="hm-header">
        <div>
          ...
          <p className="hm-subtitle">
            Duyệt, cấm và theo dõi toàn bộ hệ thống khách sạn
          </p>
        </div>
        <div className="hm-header-actions">
          <button className="hm-btn hm-btn--refresh" onClick={fetchHotels}>
            ↻ Làm mới
          </button>
      ...
      <div className="hm-stats">
        <div className="hm-stat-card">
          <span className="hm-stat-num">{stats.total}</span>
          <span className="hm-stat-label">Tổng số</span>
        </div>
        <div className="hm-stat-card hm-stat-card--approved">
          <span className="hm-stat-num">{stats.approved}</span>
          <span className="hm-stat-label">Đã duyệt</span>
        </div>
        <div className="hm-stat-card hm-stat-card--pending">
          <span className="hm-stat-num">{stats.pending}</span>
          <span className="hm-stat-label">Chờ duyệt</span>
        </div>
        <div className="hm-stat-card hm-stat-card--banned">
          <span className="hm-stat-num">{stats.banned}</span>
          <span className="hm-stat-label">Bị cấm</span>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="hm-layout">
          <div
            className={`hm-table-wrap ${selectedHotel ? "hm-table-wrap--narrow" : ""}`}
          >
            <table className="hm-table">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="hm-empty">
                  ...
                  filtered.map((hotel) => (
                    <tr
                      key={hotel.id}
                      className={`hm-row ${selectedHotel?.id === hotel.id ? "hm-row--active" : ""}`}
                    ...
                    >
                      ...
                      <td>
                        <span className="hm-city">{hotel.city}</span>
                      </td>
                      <td>
                        <span className="hm-stars">
                          {STAR_MAP[hotel.stars] || "—"}
                        </span>
                      </td>
                      <td>
                      ...
                      <td>
                        <span
                          className={`hm-badge ${statusClass(hotel.status)}`}
                        >
                      ...
                      <td onClick={(e) => e.stopPropagation()}>
                        <div className="hm-actions">
                          {/* Nút sửa hiển thị cho tất cả nếu là chủ hoặc admin */}
                          <button
                            className="hm-btn hm-btn--edit"
                            onClick={(e) => openEdit(hotel, e)}
                          >
                            ✏️
                          </button>

                          {/* Chỉ Admin mới được Duyệt/Cấm */}
                            ...
                            <>
                              {hotel.status !== "approved" && (
                                <button
                                  className="hm-btn hm-btn--approve"
                                  onClick={() =>
                                    handleAction(hotel.id, "approved")
                                  }
                                >
                                  Duyệt
                                </button>
                              ...
                              {hotel.status !== "banned" && (
                                <button
                                  className="hm-btn hm-btn--ban"
                                  onClick={() =>
                                    handleAction(hotel.id, "banned")
                                  }
                                >
                                  Cấm
                                </button>
                              ...
                              <button
                                className="hm-btn hm-btn--delete"
            ...
            <div className="hm-count">
              {filtered.length} / {hotels.length} khách sạn
            </div>
          </div>
            ...
            <div className="hm-detail">
              ...
              <div className="hm-detail-body">
                <div className="hm-detail-row">
                  <span className="hm-detail-label">Thành phố</span>
                  <span>{selectedHotel.city}</span>
                </div>
                <div className="hm-detail-row">
                  <span className="hm-detail-label">Địa chỉ</span>
                  <span>{selectedHotel.address}</span>
                </div>
                <div className="hm-detail-row">
                  <span className="hm-detail-label">Hạng sao</span>
                  <span className="hm-stars">
                    {STAR_MAP[selectedHotel.stars] || "—"}
                  </span>
                </div>
                <div className="hm-detail-row">
                  <span className="hm-detail-label">Đánh giá</span>
                  <span>⭐ {selectedHotel.rating}/10</span>
                </div>
                <div className="hm-detail-row">
                  <span className="hm-detail-label">Giá từ</span>
                  <span className="hm-price">
                    {selectedHotel.price
                      ? Number(selectedHotel.price).toLocaleString("vi-VN") +
                        "₫/đêm"
                      : "—"}
                  </span>
                </div>
                <div className="hm-detail-row">
                  <span className="hm-detail-label">Trạng thái</span>
                  <span
                    className={`hm-badge ${statusClass(selectedHotel.status)}`}
                  >
                    {statusLabel(selectedHotel.status)}
                  </span>
                </div>
                {selectedHotel.description && (
                  <div className="hm-detail-desc">
                    <span className="hm-detail-label">Mô tả</span>
                ...
                <div className="hm-detail-actions">
                  <button
                    className="hm-btn hm-btn--edit"
                    onClick={(e) => openEdit(selectedHotel, e)}
                  >
                    ✏️ Sửa thông tin
                  </button>
                  {selectedHotel.status !== "approved" && (
                    <button
                      className="hm-btn hm-btn--approve"
                      onClick={() => handleAction(selectedHotel.id, "approved")}
                    >
                      ✓ Duyệt
                    </button>
                  )}
                  {selectedHotel.status !== "banned" && (
                    <button
                      className="hm-btn hm-btn--ban"
                      onClick={() => handleAction(selectedHotel.id, "banned")}
                    >
                      ⊘ Cấm
                    </button>
                  )}
                ...
                <div className="hm-images">
                  <h3 className="hm-rooms-title">Hình ảnh khách sạn</h3>
                  {imagesLoading ? (
                    <div className="hm-rooms-loading">
                      <div className="hm-spinner hm-spinner--sm" /> Đang tải...
                    </div>
                  ) : (
                    <div className="hm-image-grid">
                      {images.map((img) => (
                        <div key={img.id} className="hm-image-item">
                          <img src={`${API}${img.image_url}`} alt="hotel" />
                          <button
                            className="hm-image-delete"
                      ...
                      {images.length === 0 && (
                        <p className="hm-rooms-empty">Chưa có ảnh nào.</p>
                      )}
                    </div>
                  )}
                  <label className="hm-btn hm-btn--add hm-upload-btn">
                    ...
                    <div className="hm-room-list">
                      {rooms.map((r) => (
                        <div key={r.id} className="hm-room-card">
                          ...
                          <div className="hm-room-price">
                            {r.base_price
                              ? Number(r.base_price).toLocaleString("vi-VN") +
                                "₫/đêm"
                              : "—"}
                          </div>
                        </div>
