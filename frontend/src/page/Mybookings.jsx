...
const MyBookings = () => {
  ...
  const getStatusColor = (status) => {
    switch(status) {
      case 'pending': return { color: '#f59e0b', label: '🟡 Đang chờ xác nhận' };
      case 'confirmed': return { color: '#10b981', label: '✅ Đã xác nhận' };
      case 'cancelled': return { color: '#ef4444', label: '❌ Đã hủy' };
      case 'completed': return { color: '#3b82f6', label: '✔️ Hoàn thành' };
    ...
    <div style={{ maxWidth: '900px', margin: '40px auto', padding: '0 20px' }}>
      ...
      {bookings.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px', background: '#f9f9f9', borderRadius: '12px' }}>
          <i className="fa-solid fa-bed" style={{ fontSize: '48px', color: '#ccc' }}></i>
          <p style={{ color: '#888', marginTop: '16px' }}>Bạn chưa có đặt phòng nào</p>
          <button 
            onClick={() => navigate('/search')}
            style={{ marginTop: '16px', padding: '10px 24px', background: '#ef5b25', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}
          >
            Tìm khách sạn ngay
          </button>
        </div>
        ...
        bookings.map((booking) => {
          ...
          return (
            <div key={booking.id} style={{
              ...
              {/* Thông tin */}
              <div style={{ flex: 1 }}>
                <h3 style={{ color: '#003580', margin: '0 0 6px 0', fontSize: '16px' }}>
                  {booking.hotel_name}
                </h3>
                <p style={{ color: '#555', fontSize: '13px', margin: '0 0 4px 0' }}>
                  <i className="fa-solid fa-bed" style={{ marginRight: '6px' }}></i>
                  {booking.room_name}
                </p>
                <p style={{ color: '#555', fontSize: '13px', margin: '0 0 4px 0' }}>
              ...
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                <p style={{ color: '#ef5b25', fontWeight: 'bold', fontSize: '16px', margin: '0 0 8px 0' }}>
                  {booking.total_amount?.toLocaleString('vi-VN')} VND
                </p>
                <span style={{
                  color: status.color, fontWeight: '600', fontSize: '13px',
                  background: `${status.color}15`, padding: '4px 10px', borderRadius: '20px'
                }}>
                  {status.label}
                </span>
              </div>
